    ...
    public class fsPrimitiveConverter : fsConverter {
        ...
        public override fsResult TryDeserialize(fsData storage, ref object instance, Type storageType) {
            ...
            if (UseDouble(storageType) || UseInt64(storageType)) {
                if (storage.IsDouble) {
                    try{instance = (float)storage.AsDouble;} //most used case
                    catch{instance = Convert.ChangeType(storage.AsDouble, storageType);}
                }
                else if (storage.IsInt64) {
                    try {instance = (int)storage.AsInt64;} //most used case
                    catch{instance = Convert.ChangeType(storage.AsInt64, storageType);}
                }
