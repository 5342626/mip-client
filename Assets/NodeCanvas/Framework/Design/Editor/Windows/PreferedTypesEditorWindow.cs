...
namespace NodeCanvas.Editor{
	...
	public class PreferedTypesEditorWindow : EditorWindow {
		...
		void OnGUI(){
			...
			if (GUILayout.Button("Generate AOT Classes")){
				if (EditorUtility.DisplayDialog("Generate AOT Classes", "This will generate a script relevant to AOT compatibility for certain platforms", "OK")){
					var path = EditorUtility.SaveFilePanelInProject ("AOT Dummy File", "AOTDummy", "cs", "");
		            if (!string.IsNullOrEmpty(path)){
						AOTClassesGenerator.Generate(path);
