	...
	abstract public class BBParameter {
		...
		public Variable varRef{
			...
			set
			{
				...
				if (_varRef != value){
					if (_varRef != null){ //remove previous
						_varRef.onNameChanged -= OnVariableNameChanged;
					}
					if (value != null){ //add new
						value.onNameChanged += OnVariableNameChanged;
					}
					_varRef = value;
					Bind(value);
				}
		...
		void OnVariableNameChanged(string newName){
		...
		public string name{
			...
			set
			{
				if (_name != value){
					_name = value;
					useBlackboard = value != null;
					varRef = value != null && bb != null? bb.GetVariable(value, varType) : null;
				}
			}
		}
		...
		new public T value{
			get
			{
				...
				if (name != null && bb != null){
					...
					//this will not create a new var but get if there exists one
					varRef = bb.GetVariable<T>(name);
