		...
		public override void OnBeforeDeserialize(Type storageType, ref fsData data){
			...
			if (json.TryGetValue("$type", out typeData)){
				...
				if (serializedType == null){
					...
					foreach(var type in ReflectionTools.GetAllTypes()){
						...
						if (att != null && att.previousTypeNames.Any(n => n == typeData.AsString) ){
							typeData = new fsData( type.FullName );
							return;
						}
					}
					...
					typeData = new fsData( missingNodeType.FullName );
				...
				if (serializedType == typeof(MissingAction) || serializedType == typeof(MissingCondition)){
					...
					var missingType = ReflectionTools.GetType( typeData.AsString );
					if (missingType != null){
						...
						typeData = new fsData( missingType.FullName );
